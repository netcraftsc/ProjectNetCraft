...
namespace NetCraft.Tests
{
    public abstract class NetCraftTestBase : AbpIntegratedTestBase<NetCraftTestModule>
    {
        ...
        protected void UsingDbContext(Action<NetCraftDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<NetCraftDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<NetCraftDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<NetCraftDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<NetCraftDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NetCraftDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<NetCraftDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NetCraftDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<NetCraftDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<NetCraftDbContext, Task<T>> func)
        {
            T result;
